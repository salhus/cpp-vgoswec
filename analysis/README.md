...
>   The committed `capture_efficiency_VGM*.csv` files currently predate the shared
>   sweep-method unification; the owner will refresh them after the longer rerun.
...
- Capture numerator: **committed current CSVs** used a half-record mean; the code
  now uses the shared method in [`../docs/SWEEP_METHOD.md`](../docs/SWEEP_METHOD.md)
  for future reruns (final `20` whole cycles, `dt = 0.01 s`, `10 + 280·T`,
  provenance columns in the refreshed CSVs).
- Optimum denominator:
