...
def plot_per_flap(passive_map: dict[int, Path], opt_map: dict[int, Path], t_res: dict[int, float], out_png: Path) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(11.0, 6.5), sharex=True, sharey=True)
    axes_flat = axes.flatten()

    for idx, angle in enumerate(sorted(FLAPS)):
        ax = axes_flat[idx]
        passive_rows = load_efficiency_csv(passive_map[angle])
        opt_rows = load_efficiency_csv(opt_map[angle])
        ...
        ax.plot(t_passive, p_passive, linestyle="--", linewidth=1.8, color="tab:blue", label="passive")
        ax.plot(t_opt, p_opt, linestyle="-", linewidth=1.8, color="tab:orange", label="opt_passive")
...
def plot_stage_gain(passive_map: dict[int, Path], opt_map: dict[int, Path], out_png: Path) -> None:
    ...
    for color, angle in zip(colors, sorted(FLAPS)):
        passive_rows = load_efficiency_csv(passive_map[angle])
        opt_rows = load_efficiency_csv(opt_map[angle])
        t = np.array([row["T_s"] for row in passive_rows], dtype=float)
        p_passive = np.array([row["P_capture_W"] for row in passive_rows], dtype=float)
        p_opt = np.array([row["P_capture_W"] for row in opt_rows], dtype=float)
        ratio = np.divide(
            p_opt,
